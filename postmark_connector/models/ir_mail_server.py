...
def convert_to_mime(email_msg: EmailMessage):
    ...
    for part in email_msg.iter_parts():
        if part.get_content_maintype() == 'multipart':
            # If it's a multipart part, iterate through its subparts
            for subpart in part.iter_parts():
                payload = subpart.get_payload(decode=True)
                content_type = subpart.get_content_type().lower()
                mime_part = payload.decode('utf-8')

                # Determine part's content type
                if 'text/plain' in content_type:
                    text_body = MIMEText(mime_part, 'plain')
                elif 'text/html' in content_type:
                    html_body = MIMEText(mime_part, 'html')
        else:
    ...
    # Raise an error if both text and HTML bodies are missing
